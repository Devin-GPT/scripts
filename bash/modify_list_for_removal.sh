...
# Specifically, it will add "rm " at the beginning of each line and " &&" at the end of each line,
...
# 'END { print "rm " last }' prints the last line without " &&" at the end.
awk 'NR > 1 { print "rm " last " &&" } { last = $0 } END { print "rm " last }' answers.txt > temp.txt && mv temp.txt answers.txt
